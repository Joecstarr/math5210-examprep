...
{ Let $G$ be a region and let $f: G \rightarrow \mathbb{C}$ be a continuous function such that $\int_{T} f=0$ for every triangular path $T$ in $G$; then $f$ is analytic in $G$.}
\boxset{Cauchy's Theorem (Second Version)}
{If $f: G \rightarrow \mathbb{C}$ is an analytic function and $\gamma$ is a closed rectifiable curve in $G$ such that $\gamma \sim 0$, then
\[\int_{\gamma} f=0 \text {. }\]}
\boxset{Cauchy's Theorem (Fourth Version)}
{If $G$ is simply connected then i $f=0$ for every closed rectifiable curve and every analytic function $f$.}
